...
## Addendum — 2026-06-26 08:19 ET

COMP-009 now adds a content-bearing `Room trail` paper/photo map cue to the shared next-step copy sheet. The continuity band should start as a visible route-map object before the two next-room cards, keeping the exit shelf aligned with content-bearing polaroid/photo-center and paper-snippet lessons without adding fake images or changing route suggestions.
...
## Addendum — 2026-06-26 15:20 ET

COMP-001 and COMP-005 now carry `app/header-identity-paper-seal-pass.css`, which turns the `Angel Berger` identity link into a clipped paper studio label with a small abstract photo/seal face. This strengthens the shared header's paper read at screenshot scale while preserving direct unnumbered navigation, accessible focus state, and the public identity copy.

## Addendum — 2026-06-26 16:48 ET

COMP-005 now retires the rendered footer `Room loop` rail because the four-room exit receipt shelf and active footer route trail already expose the same top-level routes. `app/site-chrome.jsx` now renders one calmer exit shelf plus `FooterNav`, and `app/layout.jsx` no longer loads the unused room-loop styling passes. This keeps the footer aligned with the anti-accumulation, calm-room, direct-route, and performance-budget guidance without removing any actual route access.
