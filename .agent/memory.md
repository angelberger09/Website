...
- The homepage hero now includes a `home-hero-route-strip` / `home-hero-route-slip` paper route strip in `app/page.jsx` plus `app/home-paper-collage-pass.css`, so the first screen itself gives a warm Start here path into About, Notes, Portfolio, and Store instead of making visitors wait for later door sections; `app/performance-budget-pass.css` also covers the new slips so this orientation layer stays inside the render budget.
...
- `/Website/` explains the public studio shape, highlights the filled front room, offers a first-screen Start here route strip into the four main doors, links to the four main doors again in the fuller door board, shows visible studio quality checks for paper material, human-facing copy, and honest growth, and offers a short next-step path. Its hero route strip, front-room highlights, room status, door grid, studio-shape pillars, studio checks, and reusable continuity band now render as paper/collage board systems instead of generic detail-card grids or broad shared link panels.
