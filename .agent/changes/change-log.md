...
## 2026-06-26 19:40 ET — Footer prepared Store cue

### Changed

- Updated `app/site-footer-nav.jsx` so the compact Store route-photo cue reads `Prep` instead of `Shop`.
- Updated the active Store footer state to `Current prepared store path`.
- Added `.agent/changes/2026-06-26-footer-prepared-store-cue.md` as the dedicated ledger note.

### Why

The Store footer route was correctly linked and visible, but the compact `Shop` cue could imply live inventory or open product links. Current Store readiness and public-safety guidance supports direct navigation to Store while keeping the visible cue honest about its prepared future-studio state.
