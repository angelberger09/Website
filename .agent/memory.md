...
- The homepage, Notes reader, Portfolio cards, and Store readiness rules now soften visitor-facing source language in `app/page.jsx`, `app/site-data.js`, `app/source-readiness.js`, `app/notes/NotesPageClient.jsx`, and `app/store-readiness.js`, replacing visible `repo` / raw-feed / Website-owned phrasing with warmer studio-room, public-source, and quiet-backup language while preserving the underlying source contracts.
...
- The Store page now also publishes a public-safe future feed contract so the future Store repo can expose calm fields before Website cards consume live project data.
...
- `/Website/` explains the public studio shape, highlights the filled front room, links to the four main doors, and offers a short next-step path. Its front-room highlights, room status, door grid, studio-shape pillars, and reusable continuity band now render as paper/collage board systems instead of generic detail-card grids or broad shared link panels.
