...
- Workflow reliability problems should leave public-safe recovery packets in `.agent/feedback/workflow-packets/`; the active recovery workflow should write append-only packets for failed workflow runs or manual recovery checks so failure context becomes repo memory instead of being lost in action logs.
