...
- `/Website/notes/` loads published notes from the Blog repo, explains reader behavior for loading, error, empty, and published states, names Blog source-readiness expectations, and routes visitors toward portfolio or home. Its main reader/source sections now render as a paper reader board, scrollable note strip, source ledger, and fallback tapes instead of broad generic panels.
...
- `/Website/portfolio/` gives the first public project/archive cards, archive readiness lanes, archive lanes, a future Portfolio feed contract, staged record states, a publishing rhythm, and related routes into notes or store. Its opening archive now renders as staged paper/photo pieces instead of a generic shared card grid.
