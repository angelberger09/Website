...
- The homepage hero now uses `home-hero-contact-sheet`, `home-hero-route-strip`, `home-hero-route-slip`, and `home-hero-route-slip__preview` markup in `app/page.jsx` plus `app/home-entry-contact-sheet-pass.css`, so the first screen gives a warm Start here path into About, Notes, Portfolio, and Store as lightweight mini route-photo cards rather than only text slips; the pass avoids filters, blend modes, and animated layers so it stays compatible with the render budget.
...
- Individual Notes/Blog post routes now also render real profile details as `notes-post-receipt-strip` / `notes-post-receipt-slip` paper receipts in `app/notes/post/PostReaderClient.jsx` plus `app/notes-post-receipt-strip-pass.css`, so series/category, date, and reader path metadata become visible paper snippets instead of only generic tag pills.
- Individual Notes/Blog post routes now render the read/return/continue shelf as paper/photo path cards with honest abstract visual centers through `notes-post-path-note__visual` in `app/notes/post/PostReaderClient.jsx` plus `app/notes-post-path-shelf-pass.css`, so post continuity is more image-led without implying fake note art or private content.
...
- The shared header now also loads `app/header-paper-labels-pass.css` after the active-room pass, adding a visible under-sheet, paper seams, and stronger direct-label tab depth so the identity and nav feel more like placed paper labels at screenshot scale without adding numbers, helper text, dropdowns, or a new header system.
...
- The About page now includes `about-contact-sheet-board`, `about-contact-sheet`, and `about-contact-card` markup in `app/about/page.jsx` plus `app/about-contact-sheet-pass.css`, adding a horizontal paper/photo contact-sheet sequence from existing public About orientation copy without inventing fake images or private material.
...
The shared header should give visitors a visible current-room cue while preserving direct navigation: no `Pages` button, no edge numbers, no helper text, no dropdown panel, and no heavy effects.
...
- `/Website/` explains the public studio shape, highlights the filled front room, offers a first-screen Start here contact sheet with mini route-photo cards into the four main doors, links to the four main doors again in the fuller door board, shows visible studio quality checks for paper material, human-facing copy, and honest growth, and offers a short next-step path. Its hero contact sheet, front-room highlights, room status, door grid, studio-shape pillars, studio checks, and reusable continuity band now render as paper/collage board systems instead of generic detail-card grids or broad shared link panels.
- `/Website/about/` explains the public studio shape, shows a horizontal paper/photo contact sheet from existing About orientation copy, presents working principles and next-path choices, and routes visitors toward notes or portfolio. Its opening orientation, contact sheet, principles, pathways, and reusable continuity band now render as a paper room-map collage, route-specific paper/photo sequence, taped paper-note boards, and a paper trail instead of generic `DetailCard` grids or broad shared panels.
