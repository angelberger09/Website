...
| COMP-003 | Scroll Title Sequence | PAGE-001 | Create authored arrival moment | Left-docked sticky title with subtle scroll rotation and a small content-bearing hero polaroid card plus first-screen paper route slips | Art-lettering style with polaroid material stack; when the frame appears in the hero, the center carries readable studio-room preview content, and the new `home-hero-route-strip` gives visitors immediate paper-slip room choices without waiting for lower door sections | LESSON-001, LESSON-002, LESSON-012, LESSON-014, LESSON-016, LESSON-018, LESSON-021, LESSON-022 | Motion must remain purposeful, frame content and route slips must stay readable, especially on small screens, and the performance budget should keep new hero slips cheap | active |
| COMP-004 | Studio Page Cards | PAGE-001 | Present destinations as friendly doors | First-screen route slips and fuller grid of four linked room cards with entry cues | Homepage route doors now use `home-door-board` and `home-door-card` with honest image-like centers, clipped paper surfaces, visible room labels, and accessible links rather than the broad shared `page-card` treatment; the hero now also repeats those same four destinations as small paper route slips for faster orientation | LESSON-002, LESSON-003, LESSON-015, LESSON-018, LESSON-019, LESSON-021, LESSON-022, LESSON-024, LESSON-025 | Must stay synced with page routes and avoid decorative empty card frames, fake previews, redundant noisy navigation, or overly rounded UI cards | active |
...
## Addendum — 2026-06-25 05:41 ET

COMP-009 now uses dedicated `continuity-paper-trail` markup and `app/continuity-paper-trail-pass.css` so every major route's reusable continuation area visibly reads as a staggered paper trail instead of a broad shared panel with generic pathway rows.

## Addendum — 2026-06-25 05:49 ET

COMP-008 now removes the first generic About `DetailCard` grid. The existing `aboutSections` data renders as an `about-opening-board` / `about-room-map` / `about-room-piece` collage so the About route opens with a visible paper room-map before continuing into its principles, pathways, and continuity trail.

## Addendum — 2026-06-25 06:19 ET

COMP-005 now includes the shared paper footer pass. `StudioFooter` renders a clipped studio mark and placed navigation slips, with `app/footer-paper-trail-pass.css` carrying paper texture, cut edges, staggered link placement, and responsive fallback so every route ends inside the same tactile paper/collage chrome.

## Addendum — 2026-06-25 06:39 ET

COMP-005 now simplifies the shared footer paper trail so its route slips use direct unnumbered labels, matching the current direct-navigation rule more closely while keeping the clipped paper footer structure and accessible links.

## Addendum — 2026-06-25 07:10 ET

COMP-006 and COMP-008 now carry a human-facing source-copy pass. The rendered site still explains public notes, future portfolio records, and future store sources honestly, but visible language now favors studio rooms, public writing sources, and quiet backup paths over repo/raw/Website-owned phrasing.

## Addendum — 2026-06-25 07:18 ET

COMP-005 now carries a shared PageIntro photo-card pass. `app/page-intro-photo-card-pass.css` brings the route-specific photo-center card forward between the polaroid backing and torn frame, adds tape/sheet details, and keeps route labels readable so the shared route intro motif satisfies the content-bearing polaroid rule more visibly across About, Notes, Portfolio, Store, and Blog compatibility routes.

## Addendum — 2026-06-25 07:30 ET

COMP-007 now carries a human-facing notes post source slip. `app/notes/post/PostReaderClient.jsx` removes the remaining file-viewer style source copy from the individual reader surface, and `app/notes-post-source-slip-pass.css` adds a small clipped paper slip so the public-source boundary is visible without exposing endpoint language.

## Addendum — 2026-06-25 08:28 ET

COMP-006 now carries the source-readiness detail-slip refinement. `app/notes/NotesPageClient.jsx` renders Blog readiness details as `notes-source-detail-slip` pieces instead of a generic `detail-list`, and `app/notes-paper-reader-pass.css` styles those requirements as small clipped paper slips inside the source ledger.

## Addendum — 2026-06-25 09:09 ET

COMP-003, COMP-004, and COMP-008 now carry a first-screen homepage route-orientation pass. `app/page.jsx` renders a `home-hero-route-strip` with four `home-hero-route-slip` links under the hero note, `app/home-paper-collage-pass.css` styles them as clipped paper snippets, and `app/performance-budget-pass.css` keeps their surfaces, shadows, and transforms within the final render budget.
